...
collector_t* collector_init(INIT_FUNCTION collector_internal_init_function) {
    ...
    log_info("Initialize collector, name=[%s], priority=[%d]", collector_get_name(collector_ptr), collector_get_priority(collector_ptr));
...
IOTSECURITY_RESULT collector_peek_event(collector_t* collector_ptr, event_t** event) {
    ...
    if (peeked_event == NULL) {
        log_info("Collector=[%s] is empty", collector_get_name(collector_ptr));
