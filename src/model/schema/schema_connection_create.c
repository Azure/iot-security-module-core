...
// We must ignore this cast to get a good hash function
#ifdef __GNUC__
    #pragma GCC diagnostic ignored "-Wpointer-to-int-cast"
    unsigned int result = (unsigned int)data_ptr;
#endif
#ifdef _MSC_VER
...
void schema_connection_create_log_info(connection_create_t* data_ptr) {
    ...
    log_info("ConnectionCreateSchema: local_ip=[%s], local_port=[%u], remote_ip=[%s], remote_port=[%u], protocol=[%s]",
        local_ip,
        data_ptr->local_port,
        remote_ip,
