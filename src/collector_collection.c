...
collector_collection_t* collector_collection_init() {
    log_info("Init collector collection");
    ...
    if (result != IOTSECURITY_RESULT_OK) {
        log_info("Collector collection array is not being initialized properly");
        goto cleanup;
    }
    ...
    if (result != IOTSECURITY_RESULT_OK) {
        log_info("Collector collection failed to initialize collector lists, result=[%d]", result);
        goto cleanup;
    }

    result = collector_collection_internal_init_startup_time(collector_collection_ptr);
    if (result != IOTSECURITY_RESULT_OK) {
        log_info("Collector collection failed to init collectors startup time, result=[%d]", result);
        goto cleanup;
    }
